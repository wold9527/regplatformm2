...
if [ -z "${GEMINI_URL:-}" ]; then
  echo "FATAL: GEMINI_URL 环境变量未设置，无法启动" >&2
  exit 1
fi
...
if [ -n "$GH_TOKEN" ]; then
  ...
  case "$DOWNLOAD_URL" in
    https://github.com/*)
      ...
      if [ -z "$ASSET_URL" ] || [ "$ASSET_URL" = "null" ]; then
        echo "FATAL: GitHub Release 资源未找到 (repo=$OWNER_REPO, tag=$TAG, file=$FILE_NAME)" >&2
        exit 1
      fi
  ...
  if ! curl -fsSL -A "$UA" "$GEMINI_URL" -o "$ARCHIVE_FILE"; then
    echo "FATAL: 下载代码包失败: $GEMINI_URL" >&2
    exit 1
  fi
fi
