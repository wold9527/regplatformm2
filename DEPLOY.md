...
```bash
# 1. 克隆仓库
git clone https://github.com/wold9527/regplatformm2.git
cd regplatformm2

# 2. 配置环境变量
cp .env.example .env
vim .env  # 必须修改 DB_PASSWORD、JWT_SECRET（生产环境）

# 3. （可选）如果 GHCR 包是私有仓库，先登录
docker login ghcr.io

# 4. 最小部署（推荐，NAS 场景）
docker compose -f docker-compose.prod.yml pull db app
...
### 1.3 GitHub Packages（GHCR）公开发布注意事项

1. 仓库 Settings → Packages，把镜像包改成 `Public`
2. 确认主镜像名与 compose 一致：`ghcr.io/<owner>/regplatformm2:latest`
3. NAS 仅拉取必需服务：`db` + `app`，避免拉取未发布的可选镜像
...
| **app** | `ghcr.io/<owner>/regplatformm2:latest` | 是 | 8000 | 主服务（Go 后端 + Vue 前端） |
| **xray** | `teddysun/xray:latest` | 否 | - | 代理工具（`--profile proxy` 启用） |
| **turnstile-solver** | `ghcr.io/<owner>/regplatformm2-turnstile-solver:latest` | 否 | 5072 | Turnstile 求解（`--profile worker`） |
| **aws-builder-id-reg** | `ghcr.io/<owner>/regplatformm2-aws-builder-id-reg:latest` | 否 | 5076 | Kiro Worker（`--profile worker`） |
| **cf-bypass-solver** | `ghcr.io/<owner>/regplatformm2-cf-bypass-solver:latest` | 否 | 5073 | 可选，本仓库默认不发布镜像 |
| **openai-reg** | `ghcr.io/<owner>/regplatformm2-openai-reg:latest` | 否 | 5075 | 可选，本仓库默认不发布镜像 |
| **camoufox-reg** | `ghcr.io/<owner>/regplatformm2-camoufox-reg:latest` | 否 | 5074 | 可选，`--profile worker --profile browser-mode` |
...
# 启用 camoufox-reg（可选）
docker compose -f docker-compose.prod.yml --profile worker --profile browser-mode up -d
```
...
| `build-turnstile-solver-image` | Docker 镜像 | 本地 Turnstile Worker 部署 |
| `build-aws-builder-id-reg-image` | Docker 镜像 | 本地 Kiro Worker 部署 |
